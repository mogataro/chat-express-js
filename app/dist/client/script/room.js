...
const onMessage = (event) => {
  ...
  if ((json == null ? void 0 : json.init) === true) {
    uuid = json.uuid;
    sendInitMessage();
  ...
  if ((json == null ? void 0 : json.type) === "admin_number-in-channel") {
    const channelInChannelSpan = document.querySelector(
...
const createMessage = (json) => {
  ...
  nameElement.textContent = json.uuid ? `${json.name}(${json.uuid})` : json.name;
  timeElement.textContent = dayjs(json.time).format("M/D HH:mm");
  return messageElement;
};
const getMessageModifier = (type) => {
  switch (type) {
    case "mine":
      return "message--mine";
    case "other":
...
const setUserNameAndChannel = () => {
  ...
  if (tempNumberChannel < 0 || tempNumberChannel > 99999 || Number.isNaN(tempNumberChannel)) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const roomFormEl = document.querySelector(".room__footer");
  if (roomFormEl !== null) {
